...
assessmentButton.addEventListener( // イベント検知設定の追加
  ...
  () => {  //無名関数でアロー関数、イベント検知をしたら実行される
    const userName = userNaneInput.value; // 入力欄(input)の値をの値を
    if (userName.length === 0) { // 入力が空だったら
      return; // 関数の処理を終了する。
    }

    //診断結果エリアの作成
    resultDivision.innerText = ''; //divタグを空文字列で上書きする事で、空にしている
    ...
    //bodyDivisionの作成
    const bodyDivision = document.createElement('div');//div タグ作成
    bodyDivision.setAttribute('class', 'card-body');// bootstrap 用のclass を設定
    ...
    //resultDivision.appendChild(header) //divタグの子要素として確認
    ...
    anchor.innerText = 'Post #あなたのいいところ';//ボタンの追加
...
userNaneInput,addEventListener( //イベント検知の追加
  'keydown', // キー入力
  (event) => {
    if (event.code === 'Enter') { //押されたキーがEnterだったら
